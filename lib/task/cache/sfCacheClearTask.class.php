...
EOF;
  ...
  {
    // create a lock file
    $this->filesystem->touch(sfConfig::get('sf_cache_dir').'/'.$lockName.'.lck');

    // change mode so the web user can remove it if we die
    $this->filesystem->chmod(sfConfig::get('sf_cache_dir').'/'.$lockName.'.lck', 0777);

    // remove cache files
    $this->filesystem->remove($finder->in(sfConfig::get('sf_root_dir').'/'.$subDir));

    // release lock
    $this->filesystem->remove(sfConfig::get('sf_cache_dir').'/'.$lockName.'.lck');
  }
}

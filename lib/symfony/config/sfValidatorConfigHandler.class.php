...
class sfValidatorConfigHandler extends sfYamlConfigHandler
{
    ...
    public function & execute ($configFile, $param = array())
    {
      ...
      foreach ($config['methods'] as $method => &$list)
      {
        ...
        if (!isset($methods[$method]))
        {
          ...
          if ($method != 'GET' && $method != 'POST')
          {
            ...
            $error = 'Configuration file "%s" specifies unsupported request method "%s"';
            $error = sprintf($error, $configFile, $method);
      ...
      // compile data
      $retval = "<?php\n" .
                "// auth-generated by sfValidatorConfigHandler\n" .
                "// date: %s\n%s\n%s\n?>";
    ...
    private function generateRegistration ($method, &$data, &$methods, &$names, &$validators)
    {
      ...
      foreach ($methods[$method] as $name)
      {
        ...
        $tmp    = "\t\$validatorManager->registerName('%s', %s, %s, %s, %s, %s);";
      ...
      foreach ($config['names'] as $name => &$attributes)
      {
        ...
        if (preg_match('/^([a-z0-9\-_]+)\{([a-z0-9\s\-_]+)\}$/i', $name, $match))
        {
          ...
          if (!isset($names[$parent][$subname]))
          {
            ...
            $error = 'Configuration file "%s" specifies unregistered parent "%s" or subname "%s"';
            $error = sprintf($error, $configFile, $parent, $subname);
        ...
        else
        {
          ...
          if (!isset($names[$name]))
          {
            ...
            $error = sprintf($error, $configFile, $name);
...
//        $name = trim($name);
        ...
        if (!isset($config['names'][$name]['required']))
        {
          ...
          $error = 'Configuration file "%s" specifies file or parameter "%s", but it is missing the "required" attribute';
          $error = sprintf($error, $configFile, $name);
        ...
        else if (strpos($name, '{') != false || strpos($name, '}') != false)
        {
          ...
          $error = 'Configuration file "%s" specifies method "%s" with invalid file/parameter name "%s"';
          $error = sprintf($error, $configFile, $method, $name);
    ...
    private function loadValidators (&$configFile, &$validators, &$config, &$list, &$entry)
    {
      ...
      foreach ($array as &$validator)
      {
        ...
        if (!isset($config[$validator]))
        {
          ...
          $error = sprintf($error, $configFile, $validator);
        ...
        if (isset($config[$validator]['file']))
        {
          ...
          if (!is_readable($file))
          {
            ...
            $error = sprintf($error, $configFile, $validator, $file);
